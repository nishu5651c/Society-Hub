...
test("development payment adapter never reports external success", async () => {
    await assert.rejects(() => new DevelopmentPaymentProvider().createCheckout({ paymentId: "p1", amount: 100, currency: "INR", returnUrl: "https://example.test" }), /No payment provider is configured/);
});
