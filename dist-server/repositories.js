...
export const leaseRepository = {
    ...
    async create(input) {
        if (prisma)
            return prisma.lease.create({ data: input, include: { property: true, tenant: { select: { id: true, name: true, email: true } } } });
...
export const memberRepository = {
    ...
    async create(input) {
        if (prisma)
            return prisma.member.create({ data: { ...input, unit: input.unit ?? null }, include: { user: { select: { id: true, name: true, email: true, role: true } }, property: true } });
...
export const notificationRepository = {
    ...
    async markRead(userId, id) {
        if (!prisma)
            throw new HttpError(503, "Notification persistence is not configured");
...
export const messagingRepository = {
    ...
    async create(userId, input) {
        if (!prisma)
            throw new HttpError(503, "Messaging persistence is not configured");
...
export const userRepository = {
    ...
    async create(input) {
        if (prisma)
            return prisma.user.create({ data: { ...input, role: input.role ?? "RESIDENT" } });
