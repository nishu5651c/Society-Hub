...
router.get("/health/ready", async (_req, res, next) => {
  try {
    if (!persistenceMode || persistenceMode === "development-fallback") return res.status(503).json({ status: "not_ready", reason: "database_not_configured" });
    const { prisma } = await import("./db.js");
    if (prisma) await prisma.$queryRaw`SELECT 1`;
    res.json({ status: "ready", persistence: persistenceMode });
  } catch (error) { next(new HttpError(503, "Database is not ready")); }
});
router.post("/auth/register", validate(z.object({
  ...
  try {
    if (await userRepository.findByEmail(req.body.email)) throw new HttpError(409, "Email already registered");
...
router.post("/auth/login", validate(z.object({
  ...
  try {
    ...
    if (!user || !(await verifyPassword(req.body.password, user.passwordHash))) throw new HttpError(401, "Invalid email or password");
...
router.get("/properties", requireAuth, async (_req, res, next) => { try { res.json({ data: await propertyRepository.list() }); } catch (error) { next(error); } });
router.post("/properties", validate(z.object({
  ...
  params: z.unknown(), query: z.unknown(),
})), requireAuth, requireRoles("ADMIN", "MANAGER"), async (req, res, next) => { try { res.status(201).json({ data: await propertyRepository.create(req.body) }); } catch (error) { next(error); } });
router.get("/properties/:id", requireAuth, validate(idParams), async (req, res, next) => { try { res.json({ data: await propertyRepository.find(req.params.id) }); } catch (error) { next(error); } });
router.get("/maintenance", requireAuth, async (req, res, next) => { try { res.json({ data: await maintenanceRepository.list(req.user) }); } catch (error) { next(error); } });
router.post("/maintenance", validate(z.object({
  ...
  router.get("/leases", requireAuth, async (req, res, next) => { try { res.json({ data: await leaseRepository.list(req.user) }); } catch (error) { next(error); } });
  router.post("/leases", requireAuth, requireRoles("ADMIN", "MANAGER", "STAFF"), validate(z.object({ body: leaseInput, params: z.unknown(), query: z.unknown() })), async (req, res, next) => {
    try { res.status(201).json({ data: await leaseRepository.create(req.body) }); } catch (error) { next(error); }
  });
  ...
  router.get("/payments", requireAuth, async (req, res, next) => { try { res.json({ data: await paymentRepository.list(req.user) }); } catch (error) { next(error); } });
  router.post("/payments", requireAuth, requireRoles("ADMIN", "MANAGER", "STAFF"), validate(z.object({ body: paymentInput, params: z.unknown(), query: z.unknown() })), async (req, res, next) => {
    try { res.status(201).json({ data: await paymentRepository.create(req.body) }); } catch (error) { next(error); }
  });
  ...
  router.get("/members", requireAuth, async (req, res, next) => { try { res.json({ data: await memberRepository.list(req.user) }); } catch (error) { next(error); } });
  router.post("/members", requireAuth, requireRoles("ADMIN", "MANAGER"), validate(z.object({ body: memberInput, params: z.unknown(), query: z.unknown() })), async (req, res, next) => {
    try { res.status(201).json({ data: await memberRepository.create(req.body) }); } catch (error) { next(error); }
  });

  router.get("/notifications", requireAuth, async (req, res, next) => { try { res.json({ data: await notificationRepository.list(req.user.id) }); } catch (error) { next(error); } });
  router.patch("/notifications/:id/read", requireAuth, validate(idParams), async (req, res, next) => {
    try { res.json({ data: await notificationRepository.markRead(req.user.id, req.params.id) }); } catch (error) { next(error); }
  });

  const applicationInput = z.object({ propertyId: z.string().min(1), unit: z.string().trim().min(1), notes: z.string().trim().max(2000).optional() });
  router.get("/applications", requireAuth, async (req, res, next) => { try { res.json({ data: await applicationRepository.list(req.user) }); } catch (error) { next(error); } });
  router.post("/applications", requireAuth, validate(z.object({ body: applicationInput, params: z.unknown(), query: z.unknown() })), async (req, res, next) => { try { await propertyRepository.find(req.body.propertyId); res.status(201).json({ data: await applicationRepository.create({ ...req.body, applicantId: req.user.id }) }); } catch (error) { next(error); } });
  router.patch("/applications/:id", requireAuth, requireRoles("ADMIN", "MANAGER", "STAFF"), validate(z.object({ body: z.object({ status: z.enum(["UNDER_REVIEW", "APPROVED", "REJECTED"]), notes: z.string().trim().max(2000).optional() }), params: idParams.shape.params, query: z.unknown() })), async (req, res, next) => { try { res.json({ data: await applicationRepository.review(req.params.id, req.user.id, req.body.status, req.body.notes) }); } catch (error) { next(error); } });

  router.get("/conversations", requireAuth, async (req, res, next) => { try { res.json({ data: await messagingRepository.list(req.user.id) }); } catch (error) { next(error); } });
  router.post("/conversations", requireAuth, validate(z.object({ body: z.object({ memberIds: z.array(z.string().min(1)).max(50), subject: z.string().trim().max(120).optional(), propertyId: z.string().min(1).optional(), body: z.string().trim().min(1).max(5000) }), params: z.unknown(), query: z.unknown() })), async (req, res, next) => { try { res.status(201).json({ data: await messagingRepository.create(req.user.id, req.body) }); } catch (error) { next(error); } });
  router.get("/conversations/:id/messages", requireAuth, validate(idParams), async (req, res, next) => { try { res.json({ data: await messagingRepository.messages(req.user.id, req.params.id) }); } catch (error) { next(error); } });
  router.post("/conversations/:id/messages", requireAuth, validate(z.object({ body: z.object({ body: z.string().trim().min(1).max(5000) }), params: idParams.shape.params, query: z.unknown() })), async (req, res, next) => { try { res.status(201).json({ data: await messagingRepository.send(req.user.id, req.params.id, req.body.body) }); } catch (error) { next(error); } });
  ...
  router.get("/analytics/summary", requireAuth, async (_req, res, next) => {
    try {
      if (!persistenceMode || !propertyRepository) throw new HttpError(503, "Analytics persistence is not configured");
